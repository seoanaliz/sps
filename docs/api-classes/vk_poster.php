    ...
    header("Content-Type: text/html; charset=utf-8");
        ...
        private function rec_arr($str, $array)
        {
...
//            echo '<br>текст<br>';
            if ($this->audio_id)
                foreach($this->audio_id as $vid) {
                    $check_arr[] = end(explode('_', $vid));
                }
             if ($this->video_id  )
                foreach($this->video_id as $vid) {
                    $check_arr[] = end(explode('_', $vid));
                }
            if ($this->link) {
                $check_arr[] = $this->link;
    ...
    //        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Expect:'));
       ...
       //возвращаемые значения
        ...
        public function send_post()
        {
            ...
            $attachment = array();
            //цикл на повторные попытки отправки
            while (1) {
                $try_cntr += 1;
                if ($try_cntr > 3) {
                    throw new exception("Error wall post");
                }
            ...
            //другие аттачи
                ...
                while (true) {
                    ...
                    if ( $capcha_tries > self::FALSE_COUNTER ) {
                        throw new exception('please change admin');
                    }
        ...
        //                print_r($fwd3);
                ...
                echo 'аттачи = ' .  $attachment . ',' . $other_attachments . '<br>';
                $this->final_attachments = trim($attachment,',') . ',' . trim($other_attachments,',');
                $check = $this->send_checker();

                if($check)
                    return $this->vk_group_id . '_' . $check;
            }

        }

        public function send_checker()
        {
            sleep(2);
            $url = self::METH . 'wall.get';
            ...
            $fwd = json_decode($this->qurl_request($url, $params));
            if (isset($fwd->error))
                throw new exception($fwd->error->error_msg);
            $fwd = $fwd->response;
            $url = self::METH . 'getServerTime';
            $fwd2 = json_decode($this->qurl_request($url, array()));
            $vk_time = $fwd2->response;
            unset($fwd[0]);

            $check_arr = $this->get_attaches();
            //everything became chaos
//            $check_arr[] = 'ljkfh;asdfak;sdfasdf';
            if (self::TESTING) {
                echo '<br>';
                print_r($check_arr);
                echo '<br>';
            }
            $quan = count($check_arr);


            //проверяем каждый пост на соответствие отправленному
            foreach ($fwd as $post) {
                $post_id = $post->id;
                $counter = 0;
                if ($vk_time - $post->date > 60) {
//                    echo 'Нет поста!<br>';

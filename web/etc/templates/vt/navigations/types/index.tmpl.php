<?php
    ...
    $__pageTitle = LocaleLoader::Translate( "vt.screens.navigationType.list");

    $grid = array(
        "columns" => array(
           LocaleLoader::Translate( "vt.navigationType.title" )
            , LocaleLoader::Translate( "vt.navigationType.alias" )
            , LocaleLoader::Translate( "vt.navigationType.statusId" )
        )
        , "colspans"	=> array()
        , "sorts"		=> array(0 => "title", 1 => "alias", 2 => "statusId")
        , "operations"	=> true
        , "allowAdd"	=> true
	...
	<div class="inner">
		...
		<div class="search<?= $hideSearch == "true" ? " closed" : ""  ?>">
			...
			<form class="search-form" id="searchForm" method="post" action="{$grid[basepath]}">
                ...
                <div class="row">
                    <label>{lang:vt.navigationType.title}</label>
                    <?= FormHelper::FormInput( "search[title]", $search['title'], 'title', null, array( 'size' => 80 ) ); ?>
                </div>
                <div class="row">
                    <label>{lang:vt.navigationType.statusId}</label>

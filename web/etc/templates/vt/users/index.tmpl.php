<?php
    ...
    $__pageTitle = LocaleLoader::Translate( "vt.screens.user.list");

    $grid = array(
        "columns" => array(
           LocaleLoader::Translate( "vt.user.login" )
            , LocaleLoader::Translate( "vt.user.statusId" )
        )
        , "colspans"	=> array()
        , "sorts"		=> array(0 => "login", 1 => "statusId")
        , "operations"	=> true
        , "allowAdd"	=> true
		...
		<div class="search<?= $hideSearch == "true" ? " closed" : ""  ?>">
			...
			<form class="search-form" id="searchForm" method="post" action="{$grid[basepath]}">
                ...
                <div class="row">
                    ...
                    <?= FormHelper::FormInput( "search[login]", $search['login'], 'login', null, array( 'size' => 40 ) ); ?>
                </div>
                <div class="row">
...
?>
			<tr data-object-id="{$id}">
                <td class="header">{$object.login}</td>

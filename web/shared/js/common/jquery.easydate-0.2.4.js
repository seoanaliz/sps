(function($) {
    ...
    function __(key, value, settings) {
        ...
        result = settings.locale[key] || key;

        if (settings.locale == $.easydate.locales.ruRU) {
    ...
    var format_date = $.easydate.format_date = function(date, options) {
        ...
        if ((!settings.format_future && diff < 0) || (!settings.format_past && diff > 0)) {
            return;
        }
        ...
        for(var i in settings.units) {
            ...
            if (diff_abs < unit.limit) {
                ...
                var format_string;
                if (diff < 0) {
                    format_string = __('future_format', NaN, settings).replace('%s', __('in', NaN, settings));
                } else {
                    format_string = __('past_format', NaN, settings).replace('%s', __('ago', NaN, settings));
                }
                return format_string.replace('%t', val + ' ' + __(unit.name, val, settings));
            }
        }
            ...
            if (diff_abs < unit.limit) {
                ...
                if (isNaN(unit.in_seconds)) {
                    ...
                    if (diff < 0) {
                        ...
                        return (unit.in_seconds - (diff_abs % unit.in_seconds)) * 1000 + 100;
                    }
                }
            }
    ...
    function get_date(element, settings) {
        ...
        if (isNaN(date)) {
            var timestamp;
            var time = settings.date_parse(timestamp = element.attr('title')) 
                    || settings.date_parse(timestamp = element.text());
    ...
    function update_time(element) {
        ...
        var element_id = element.data('easydate.id');
        ...
        if (settings.live) {
            ...
            if (!isNaN(timeout)) {
                if (timeout > 2147483647) { // max Firefox timeout delay
                    timeout = 2147483647;
                }
    ...
    $.fn.easydate = function(options) {
        ...
        this.each(function() {
            ...
            var element_id = $(this).data('easydate.id', id++);
            if (!isNaN(updates[element_id])) {
                clearTimeout(updates[element_id]);
                delete updates[element_id];
            }

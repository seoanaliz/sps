...
$.extend(Datepicker.prototype, {
	...
	show: function(target) {
		...
		if (inst) {
			...
			inst.div.css({left: offset.left - (inst.div.width() / 2) + ($(target).width() / 2), top: offset.top});
			// And display
	...
	_checkOffset: function(inst) {
		...
		var tooHigh = (offset.top + inst.target.outerHeight() + inst.div.outerHeight() - scrollY) > browserHeight;
		if (alignment == 'topLeft') {
			...
			offset = {left: (isRTL || tooWide ? alignR : alignL), top: (tooHigh ? above : below)};
            if (tooHigh) {
                inst.div.addClass('top');
            } else {
                inst.div.addClass('bottom');
            }
		}
	...
	_generateMonth: function(target, inst, year, month, renderer, first) {
		...
		monthHeader = (first && false ?
            this._generateMonthSelection(inst, year, month, minDate, maxDate, monthHeader, renderer) :

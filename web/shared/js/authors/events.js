...
//        });
    ...
    get_publics: function(callback) {
        simpleAjax('getPublics', function(rawData) {
            var clearData = [];
            var commonList = [];
            $.each(rawData.groups, function(i, rawListItem) {
    ...
    get_authors: function(listId, offset, limit, callback) {
        var params = {};
        params = {
            groupId: listId
        }
        simpleAjax('getAuthors', params, function(rawData) {
            var clearData = [];
            $.each(rawData.authors, function(i, rawAuthor) {
                var clearAuthor = Cleaner.author(rawAuthor);
                clearData.push(clearAuthor);
            });
...
var Cleaner = {
    ...
    metrick1:function(rawMetricks){
        ...
        return clearMetricks;
    },

    author: function(rawAuthor) {
        var userModel    = new UserModel(this.user(rawAuthor.user));
        var metricsModel = new MetricsModel(this.metrick1(rawAuthor.metrick1));
        return {
            id: rawAuthor.id,
            user: userModel.data(),
            metrick1: metricsModel.data(),
        };
    },

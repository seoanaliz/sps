...
if (!defined('MINIFY_BASE_DIR')) {
   ...
   */
  define('MINIFY_BASE_DIR', __ROOT__ );
}

if (!defined('MINIFY_CACHE_DIR')) {
  /** Directory where compressed files will be cached. */
  define('MINIFY_CACHE_DIR', __ROOT__ . "/" .  CONFPATH_CACHE );
}
  ...
  public function combine($minify = true) {
    ...
    foreach($this->files as $file) {
      if ($this->type === self::TYPE_CSS && MINIFY_REWRITE_CSS_URLS) {
        ...
        $combined[] = self::rewriteCSSUrls(file_get_contents($file), dirname($file));
      }
      else {
    ...
    $combined = $minify ? self::run(implode("\n", $combined), $this->type) :
        implode("\n", $combined);

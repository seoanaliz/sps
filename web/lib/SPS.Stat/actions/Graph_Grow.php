<?php
      Package::Load( 'SPS.State' );
      
    class graph_grow extends wrapper 
    {
         public function execute()
         {
            $this->db_wrap_pg('connect');
...
//            $sql = 'select * from gr50k, publs50k where gr50k.id=publs50k.vk_id';
            echo $sql . '<br>';
        $res = $this->db_wrap_pg('query',$sql);
         
        $resul = array();
        $time_for_table = 0;
        $rest = $this->q_result;
        while ($row = $this->db_wrap_pg('get_row', $rest)) {
//            print_r($row);
            ...
            while ($row2 = $this->db_wrap_pg('get_row')) {
                print_r($row2);
        ...
        echo '<table id="tbld" class="tablesorter"  >
                ...
                <tr>
                    <th>РђРІР°</th>
                    <th>РџР°Р±Р»РёРє</th>
                    <th>' .'РџРѕРґРїРёСЃС‡РёРєРѕРІ РЅР° '. date('d-m-Y', $time_for_table)  . '</th>
                    <th>РџСЂРѕС†РµРЅС‚ СЂРѕСЃС‚Р°</th>
                    <th>РђР±СЃРѕР»СЋС‚РЅС‹Р№ СЂРѕСЃС‚</th>
                </tr>
                </thead>
                <tbody>';
...
//            echo    $time_last. ' >'. $time_comparison;
//                die();

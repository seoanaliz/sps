            ...
            public function Execute()
                {
                    ...
                    $ok = new Odnoklassniki(
                        array(
                            'client_id' => '199880192',
                            'application_key' => 'CBAPOKOMABABABABA',
                            'client_secret' => '438748ACE757485AE61565E2'
                        )
                    );

                    // закомментировать для получения нового
                    $token = '{"token_type":"session","refresh_token":"58829992374048be977547a1e88400ac64ddb828f_560668414339_138","access_token":"31ipa.106mzk006r2y563e1a4f3k3nsrvd8","expires":1383301728}';
                    if($token) {
                        $ok->setToken($token);
                        $ok->refreshToken();
    //                    print_r($ok->getTokenStr());
    //                    die();
                    } else {
                        $ok->setRedirectUrl('http://beta.socialboard.ru/int/controls/setPostPrice/');
                        if(isset($_GET['code'])) {
                            $ok->getToken($_GET['code']);
                            print $ok->getTokenStr();
                        } else {
                            print '<a href="' . $ok->getLoginUrl(array('VALUABLE ACCESS', 'SET STATUS','PUBLISH TO STREAM')) . '">Login!</a>';
                            exit();
                        }
                    }
                    ...
                    print_r( $ok->api('users.setStatus', [
    //                    'uid'      =>  $uid,
                        'status'   =>  'выаыаы',
                    ]));
                    ...
                    if (isset($_GET['code'])) {
                        $params = array(
                            'code' => $_GET['code'],
                            'redirect_uri' => 'beta.socialboard.ru/int/controls/setPostPrice/',
                            'grant_type' => 'authorization_code',
                            'client_id' => 199880192,
                            'client_secret' => '438748ACE757485AE61565E2'
                        );
                    ...
                    echo '<a href="http://www.odnoklassniki.ru/oauth/authorize?client_id=199880192&response_type=code&redirect_uri=beta.socialboard.ru/int/controls/setPostPrice/">link</a>';
                    die();
                    $search = array(

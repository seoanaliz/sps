...
//пару раз в день
    ...
    public function get_posts($offset = 0)
    {
        ...
        $sql = 'SELECT MAX(time_st) FROM posts_for_likes where vk_id like \'' . $this->id. "_%'";
        $this->db_wrap('query', $sql);

        $row = $this->db_wrap('get_row');
        if (!$row || self::BASE_RENEW)
            $date_from = 0;
        else
...
//            echo 'abyrvalg';
        ...
        foreach($res as $rr) {
            ...
            //условия отброса поста по времени
            if ($rr->date > $date_to) {
...
//
            $sql = 'INSERT INTO posts_for_likes(vk_id,time_st,comments,likes,reposts)VALUES (' . $values . ')';
             $this->db_wrap('query', $sql);
        }

...
deploy:
	...
	@if [ -n "$(deploy.$(layout).hosts)" ]; then \
		for i in $(deploy.$(layout).hosts); do \
			rsync -Cavuz --chmod=ugo=rwX -e "ssh -p$(deploy.$(layout).port)" $(deploy.$(layout).params) $(deploy.$(layout).ignore) $(deploy.$(layout).dir)/ "$(deploy.$(layout).user)"@$$i:"$(deploy.$(layout).root)"; \
		...
		rsync -Cavuz --chmod=ugo=rwX -e "ssh -p$(deploy.$(layout).port)" $(deploy.$(layout).params) $(deploy.$(layout).ignore) $(deploy.$(layout).dir)/ "$(deploy.$(layout).user)"@$(deploy.$(layout).host):"$(deploy.$(layout).root)"; \
